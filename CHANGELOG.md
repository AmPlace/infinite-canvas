...
+ [新增] 线上部署升级为阿柴 AI 托管创作台，支持 NewAPI 授权门禁、站点品牌、安全充值与控制台入口、连接状态及生成后自动刷新的多币种余额展示。
+ [修复] 托管创作台恢复提示词库、我的资产、Agent、画布插件及文本/音频生成，并继续隐藏底层 API 渠道配置。
+ [优化] NewAPI 一键应用地址和 Key 后自动同步当前 Key 可用模型，并自动保留或选择默认生图、视频模型。
+ [修复] 托管创作台每次进入都会重新同步当前 Key 的真实模型并移除已下架项，支持清除旧授权和本地 Key 后返回控制台切换 Key。
+ [修复] 托管模型同步后会同步修正通用及视频模型选择，避免旧模型触发页面崩溃，允许已创建的视频任务在模型下架后继续查询，并提供友好的页面异常恢复入口。
+ [修复] OpenAI Video 兼容渠道支持 NewAPI 公开任务 ID、完整状态与真实进度，完成后通过携带当前 Key 的 content 接口获取视频。
